"""
Elasticsearch client for alert and vulnerability queries.

Wazuh alerts and vulnerability data are stored in Elasticsearch.
This client queries:
  - wazuh-alerts-* — alert data
...
# Index patterns for Wazuh data stored in Elasticsearch
...
class ElasticSearchClient:
    """
    Client for querying Elasticsearch where Wazuh data is stored.
    """
    ...
    def __init__(
        ...
        # Detect scheme from host before normalizing (http:// → no SSL)
        self._scheme = self._detect_scheme(host, verify_ssl)
    ...
    @staticmethod
    def _detect_scheme(host: str, verify_ssl: bool) -> str:
        """Detect whether to use http or https from the host string."""
        if host and host.lower().startswith("http://"):
            return "http"
        return "https"
    ...
    def base_url(self) -> str:
        ...
        return f"{self._scheme}://{self.host}:{self.port}"
    ...
    async def initialize(self):
        ...
        logger.info(f"ElasticSearchClient initialized for {self.host}:{self.port}")
    ...
    async def _search(
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Invalid JSON response from Elasticsearch: {index}")

        except httpx.HTTPStatusError as e:
            logger.error(f"Elasticsearch search failed: {e.response.status_code} - {e.response.text}")
            if e.response.status_code >= 500:
                # Let server errors propagate so tenacity retry can see them
                raise
            raise ValueError(f"Elasticsearch query failed: {e.response.status_code}")
    ...
    async def get_vulnerabilities(
        ...
        if severity:
            # Normalize severity to match Elasticsearch format
    ...
    async def get_vulnerability_summary(self) -> Dict[str, Any]:
        ...
        body = {
            "size": 0,
            "aggs": {
                "by_severity": {"terms": {"field": "vulnerability.severity.keyword", "size": 10}},
                "by_agent": {"cardinality": {"field": "agent.id.keyword"}},
                "total_vulnerabilities": {"value_count": {"field": "vulnerability.id.keyword"}},
            },
        }
...
class ElasticSearchNotConfiguredError(Exception):
    """Raised when Elasticsearch is not configured but required."""

    def __init__(self, message: str = None):
        default_message = (
            "Elasticsearch not configured. "
            "Alert and vulnerability tools require Elasticsearch.\n\n"
